...
def app():
    ...
    df = read_file('../data/category_price.csv')
...
################################################################################################################################ Displaying of graphs ####################################################################################
    if(product == 'Household'):
        ...
        max_y = max(ped) + max(ped)*0.2
        min_y = min(ped) + min(ped)*0.2

        p = figure(x_range=item, y_range=(min_y,max_y), plot_height=500, title="PED Value of each Product",
